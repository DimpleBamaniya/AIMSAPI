...
public class ProductService : IProductService
{
    ...
    public ProductService(IProductRepository productRepository, IMapper mapper, ILogger<ProductService> logger, ICommonService commonService)
    {
    ...
    public async Task<Result> GetAllProductDetailsAsync(Pagination pagination)
    {
        _logger.LogInformation("Started GetAllProductDetailsAsync with Pagination: {Pagination}", pagination);
        try
        {
            _logger.LogDebug("Fetching all product details from the repository with Pagination: {@Pagination}", pagination);
            List<ProductDetails> productData = (await _productRepository.GetAllProductDetailsAsync(pagination)).ToList();
            _logger.LogDebug("Successfully fetched product details. Total products retrieved: {Count}", productData.Count);

            _logger.LogInformation("Completed GetAllProductDetailsAsync with Pagination: {Pagination}", pagination);
            ...
            return new Result
            {
                ...
                ResultObject = productData
            };
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "An error occurred while fetching all product details with Pagination: { Pagination}", pagination);
            return new Result("An error occurred while fetching all accounts", "PRODUCTS_FETCH_ERROR", HttpStatusCode.InternalServerError, ex);
        }
    }
    ...
    public async Task<Result> SaveProductAsync(SaveProductDto productModel)
    {
        try
        {
            _logger.LogInformation("Started SaveProductAsync for ProductModel: {@ProductModel}", productModel);

            Products? product = null;
            var userId = _commonService.GetLoggedUserId();

            #region API Validations
            _logger.LogDebug("Validating fields for SaveProductAsync.");
            ...
            product = await _productRepository.SaveProductAsync(product);
    ...
    public async Task<Result> GetUserListByProductIDAsync(int id)
    {
        _logger.LogInformation("Started GetUserListByProductIDAsync with ProductID: {ProductID}", id);
        try
        {

            _logger.LogDebug("Fetching user list by ProductID: {ProductID}", id);
            List<UserByProductID> userList = (await _productRepository.GetUserListByProductIDAsync(id)).ToList();
            _logger.LogDebug("Successfully fetched user list. Total users retrieved: {Count}", userList.Count);

            _logger.LogInformation("Completed GetUserListByProductIDAsync for ProductID: {ProductID}", id);
    ...
    public async Task<Result> DeleteProductAsync(DeleteProductDto productModel)
    {
        _logger.LogInformation("Started DeleteProductAsync for ProductModel: {@ProductModel}", productModel);
        try
        {
            ...
            product = await _productRepository.DeleteProductAsync(product);
            ...
            _logger.LogInformation("Completed DeleteProductAsync for ProductId: {ProductId}", productModel.Id);
            return new Result
            {
                Status = StatusType.Success,
                HttpStatusCode = HttpStatusCode.OK,
